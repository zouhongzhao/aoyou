 ...
 */
 class Tong_remark extends CI_Controller{
 	function __construct(){
 		parent::__construct();
 		$this->myauth->execute_auth();	//验证是否登陆
 		$this->load->model('Usersmodel');
 		$this->load->model('Tong_versionmodel');
 	}
 	...
 	function day(){
		$data = null;
 		...
 		if(!empty($remark))
 		{
 			$this->session->set_userdata('remark',$remark);
 			if($this->session->userdata('remark')!=$remark) $this->session->unset_userdata('remark');
 		}
 		...
 		if($this->session->userdata('remark')) $this->db->like('remark',$this->session->userdata('remark'));
		...
		//$channels_id = $this->db->select("id,us_name")->from('users')->where('us_type','channels')->where('under','1')->get()->result_array();
 		...
 		$this->db->from("tong_day_remark");
 		$this->db->order_by('dates','desc');
 		$data = $this->mydb->getList();
//  		print_r($data);exit();
 		$this->mypage->loadview('tong_remark/day',$data);

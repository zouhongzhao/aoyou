 ...
 */
 class Tong_market extends CI_Controller{
 	 ...
 	 */
 	function day(){
 		$this->mypage->loadview('tong_market/day');
 	}
...
//  		   		print_r($_GET);
 		   	...
 		   	case 1:
 		   		...
 		   		{
 		   		$sql1 = "SELECT t.id FROM tong_day as t left join users as u on u.id=t.c_id WHERE u.us_type='channels' and u.under='0'".$wh."GROUP BY t.dates,t.c_id";
 		   		}
 		   		else
 		   		{
...
//  		   		$result = $this->db->query("SELECT COUNT(*) AS count FROM tong_day as t left join users as u on u.id=t.c_id WHERE u.us_type='channels' and u.under='0'".$wh)->result_array();
 		   		$sql1="SELECT t.id FROM tong_day as t left join users as u on u.id=t.c_id WHERE u.us_type='channels' and u.under='0' and concat(year(t.dates),'-',month(t.dates))='$date' and t.c_id='$b[1]'".$wh." GROUP BY t.dates,t.c_id";
 		   		}
...
//  		   			$total += $row[total];
					...
					$amount=0;
 		   			...
 		   			$responce->rows[$k]['cell']=array($v[c_id],$v[us_name],$v[dates],$v[activations],$v[install_nums],$v[starts_nums],$v[ip_nums],$amount,$v[shouyiss],$v[belongs_market]);
//  		   			$i++;
 		   		}
 		  ...
 		  case 2:
 		  	...
 		  	$sql1 = "SELECT t.id FROM tong_day as t left join users as u on u.id=t.c_id WHERE u.us_type='channels' and u.under='0'".$wh."group by concat(year(t.dates),'-',month(t.dates)),t.c_id";
 		  	//  		   		print_r($result);exit();
 		  	$count = $this->db->count_all("($sql1)as t ");
...
//  		   			$total += $row[total];
 		   			...
 		   			$responce->rows[$k]['cell']=array($v[u_id],$v[dates],$v[activation],$v[install_num],$v[starts_num],$v[mac_num],$v[ip_num],$v[shouyis],$v[id]);
//  		   			$i++;
 		   		}
